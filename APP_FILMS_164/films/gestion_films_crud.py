    ...
    if request.method == "POST":
        try:
            if form_add_film.validate_on_submit():
                ...
                strsql_insert_film = """INSERT INTO t_moto (id_moto,marque_moto) VALUES (NULL,%(value_nom_film)s) """
...
def film_update_wtf():
    ...
    try:
        ...
        if form_update_film.validate_on_submit():
            ...
            str_sql_update_nom_film = """UPDATE t_moto SET marque_moto = %(value_nom_film)s,
                                                            modèle_moto = %(value_duree_film)s,
                                                            nombre_km_moto = %(value_description_film)s,
                                                            prix_moto = %(value_cover_link_film)s,
                                                            année_moto = %(value_datesortie_film)s
                                                            WHERE id_moto = %(value_id_film)s"""
            with DBconnection() as mconn_bd:
            ...
            str_sql_id_film = "SELECT * FROM t_moto WHERE id_moto = %(value_id_film)s"
            ...
            print("data_film ", data_film, " type ", type(data_film), " genre ",
                  data_film["marque_moto"])

            # Afficher la valeur sélectionnée dans le champ du formulaire "film_update_wtf.html"
            form_update_film.nom_film_update_wtf.data = data_film["marque_moto"]
            form_update_film.duree_film_update_wtf.data = data_film["modèle_moto"]
            # Debug simple pour contrôler la valeur dans la console "run" de PyCharm
            print(f" duree film  ", data_film["modèle_moto"], "  type ", type(data_film["modèle_moto"]))
            form_update_film.description_film_update_wtf.data = data_film["nombre_km_moto"]
            form_update_film.cover_link_film_update_wtf.data = data_film["prix_moto"]
            form_update_film.datesortie_film_update_wtf.data = data_film["année_moto"]
...
def film_delete_wtf():
    ...
    try:
        ...
        if form_delete_film.submit_btn_del_film.data:
            ...
            str_sql_delete_fk_film_genre = """DELETE FROM t_pers_moto WHERE fk_moto = %(value_id_film)s"""
            str_sql_delete_film = """DELETE FROM t_moto WHERE id_moto = %(value_id_film)s"""
        ...
        if request.method == "GET":
            ...
            str_sql_genres_films_delete = """SELECT * FROM t_moto WHERE id_moto = %(value_id_film)s"""
